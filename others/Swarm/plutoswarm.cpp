...
int main(int argc, char **argv){
  ...
  //add IPs here
  all_ips.push_back("192.168.117.107");
  all_ips.push_back("192.168.117.52");
  ...
  for(i = 0; i < all_ips.size(); i++){
    ...
    topic_ap_name[strlen(topic_ap_name)-1] = 0x30+i;
    cout<<topic_name<<endl;
    cout<<topic_ap_name<<endl;
    usleep(2000);
    ...
    if(i==0){
      s1 = n.subscribe(topic_name, 1000, readDroneCommand);
      s1_a = n.subscribe(topic_ap_name, 1000, readDroneAPCommand);
    }
    else {
      s2 = n.subscribe(topic_name, 1000, readDroneCommand);
      s2_a = n.subscribe(topic_ap_name, 1000, readDroneAPCommand);
    }
    ...
    if (rc)
    {
      cout << "Error:unable to create communication thread," << endl;
      exit(-1);
    }
    ...
    if(isSocketCreate)
    {
      ...
      if (rc)
      {
        cout << "Error:unable to read create thread," <<  endl;
        exit(-1);
      }
      ...
      if (rc)
      {
        cout << "Error:unable to service create thread," << endl;
        exit(-1);
      }
    }
