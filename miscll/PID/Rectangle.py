...
class PID:
    ...
    def __init__(self,K_z: list, K_roll: list, K_pitch: list, K_yaw: list, dt:float, tau:float, alpha:float,cam_orientation:float,XT:float,YT:float,ZT:float) -> None:
        ...
        self.df = pd.DataFrame(columns=['rc_th','rc_r','rc_p','rc_y','sen_y','cam_x','cam_y','cam_z''drone_error_x','drone_error_y'])
...
#         self.min = min
    ...
    def controller_out(self):
        #while (math.sqrt((self.cam_err_x-self.aim_x)**2+(self.aim_y-self.cam_err_y)**2)>self.tolerance):
        if self.t_step==0:
            self.aim_x=0
            self.aim_y=0
        if self.t_step==1:
        ...
        if math.sqrt((self.cam_err_x)**2+(self.cam_err_y)**2)>self.tolerance:
            self.t_step+=1
        ...
        lst=[userRC.rcThrottle,
             userRC.rcRoll ,
             userRC.rcPitch ,
             userRC.rcYaw ,relative_yaw,
             userRC.current_x,
             userRC.current_y,
             userRC.current_z,
             -self.cam_err_x*math.cos(math.pi/180*(relative_yaw))+self.cam_err_y*math.sin(math.pi/180*(relative_yaw)),
             (self.cam_err_x*math.sin((relative_yaw)*math.pi/180)+self.cam_err_y*math.cos(math.pi/180*(relative_yaw))),time.time()]
